...
(async () => {
  ...
  try {
    for (const viewport of [{ width: 1440, height: 1000, columns: 3 }, { width: 390, height: 1100, columns: 1 }]) {
      ...
      const appliedSliceColour = await page.locator('.dashboard-heat-link').first().locator('.dashboard-heat-slice').nth(1).evaluate(node => getComputedStyle(node).fill);
      assert.equal(appliedSliceColour, appliedColour, 'applied map slice uses the matching Jobs chart colour');
      assert.equal(await page.locator('.dashboard-heat-link').first().locator('.dashboard-heat-slice').first().evaluate(node => getComputedStyle(node).fill), 'rgb(255, 255, 255)', 'job without application is white');
      assert.equal(await page.locator('.dashboard-heat-outline').count(), 2);
