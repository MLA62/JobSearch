...
(async () => {
  ...
  try {
    for (const viewport of [{ width: 1440, height: 1000, columns: 3 }, { width: 390, height: 1100, columns: 1 }]) {
      ...
      const jobsColour = await page.locator('.dashboard-chart-swatch').first().evaluate(node => getComputedStyle(node).backgroundColor);
      const bubbleColour = await page.locator('.dashboard-heat-point').first().evaluate(node => getComputedStyle(node).fill);
      assert.equal(bubbleColour, jobsColour, 'map bubbles use exactly the first Jobs chart colour');
      assert.equal(await page.locator('.dashboard-heat-point').first().evaluate(node => getComputedStyle(node).opacity), '1');
