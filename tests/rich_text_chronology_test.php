...
checkRich(str_contains($source, "const editingSource = shell.classList.contains('is-source')") && str_contains($source, 'sanitize(editingSource ? source.value : editor.innerHTML)') && str_contains($source, 'if (editingSource) editor.innerHTML = clean'), 'HTML source changes are committed back to the WYSIWYG editor');
checkRich(str_contains($source, "if(shell.classList.contains('is-source')){sync();shell.classList.remove('is-source')") && !str_contains($source, "source.addEventListener('input',()=>{if(shell.classList.contains('is-source'))editor.innerHTML"), 'Source is sanitized once on mode switch instead of while incomplete HTML is typed');
