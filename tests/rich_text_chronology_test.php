...
checkRich(str_contains($source, "command('¶',labels.paragraph,()=>convertSelectedBlocks(false))") && str_contains($source, "command('↵',labels.softBreak,()=>convertSelectedBlocks(true))"), 'Mini editor exposes selection-wide paragraph and soft-break conversion');
...
checkRich(str_contains($source, 'const labelSets = {') && str_contains($source, "fr:{paragraph:'Convertir les lignes sélectionnées en paragraphes'") && str_contains($source, "en:{paragraph:'Convert selected lines to paragraphs'"), 'Mini editor follows the user language');
